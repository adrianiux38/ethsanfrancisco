...
receive() external payable  { 
  ...
  function newApplication(
    ...
    //invokeFrom Transfer collateral to contract from borrower
    ...
    for(uint i = 0; i < _collateralTokens.length; i++) {
      safeTransfer.(_collateralTokens[i], msg.sender, address(application), _collateralAmounts[i]);
    }
  ...
  */
  ...
  function markAsLender(address lender) external {
    ...
    if(loansByLenderMap[lender][msg.sender] == false) {
      ...
      //loansByLender[lender].push(payable(msg.sender));
    }
  }
